...
def diagnose_issue():
    ...
    try:
        ...
        print(
            "\033[91mConnection Error Occurred, Proxy could be blocking connection. \033[0m"
        )
    ...
    try:
        ...
        print(
            "\033[91mIPv4 network connectivity issue. Check network settings or firewall.\033[0m"
        )
    ...
    if platform.system() != "Windows":  # Windows does not support IPv6 ping easily
        try:
            subprocess.run(
                ["ping", "-c", "1", "-6", "ipv6.google.com"], timeout=10, check=True
            )
            print("\033[92mIPv6 network connectivity is fine.\033[0m")
        except subprocess.CalledProcessError:
            print(
                "\033[91mIPv6 network connectivity issue. Check network settings or firewall.\033[0m"
            )
    ...
    try:
        ...
        print(
            "\033[91mUnable to ping. Probably Internet is not working, Check firewall settings if any.\033[0m"
        )
    except subprocess.TimeoutExpired:
...
def restart_wifi():
    ...
    if system == "Windows":
        ...
        try:
            subprocess.run(
                ["sudo", "systemctl", "restart", "network-manager"], check=True
            )
        except subprocess.CalledProcessError as e:
        ...
        try:
            ...
            time.sleep(5)
            subprocess.run(
                ["networksetup", "-setairportpower", "en0", "on"], check=True
            )
        except subprocess.CalledProcessError as e:
