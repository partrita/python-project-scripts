...
def hextodec(s):
    ...
    for p in l:
        ...
        for c in dcp:
            ...
            fh.write(
                f"\t----{16**dcp.index(c)} is 16 to the power of the index of the character in the input\n"
            )
    ...
    for i in fdl:
        ...
        fh.write(
            f"\t----{c} is final character in base64 list using decimal conversion of 6-bit binary word as index\n"
        )
        fl.append(c)
    fw = list("".join(fl))
    for i in range(3):
