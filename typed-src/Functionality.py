...
def check_connectivity(
    link,
):  # Referred website: codespeedy (https://www.codespeedy.com/how-to-check-the-internet-connection-in-python/)
...
def dwn(download_link, video_resolution, directory):
    ...
    try:
        ...
        msg.showerror(title="Downtube", message="No video resolution found")
    if video_resolution == "360p":
        try:
            video = yt.streams.filter(
                progressive=True, file_extension="mp4", res="360p", type="video"
            ).first()
            if video == None:
        ...
        try:
            video = yt.streams.filter(
                progressive=True, file_extension="mp4", res="720p", type="video"
            ).first()
            if video == None:
                raise AttributeError
            else:
...
def download_bt(
    download_link, get_link, video_resolution, directory, get_dir, resolution_box
):
    """All the process for downloading will be done here"""
    verify_YT = re.search(
        r"youtube.com", download_link
    )  # Verifying youtube link or not
