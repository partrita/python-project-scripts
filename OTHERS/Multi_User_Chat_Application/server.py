        ...
        while True:
            ...
            if not message:
                ...
                if recipient in nicknames.values():
                    recipient_socket = list(nicknames.keys())[
                        list(nicknames.values()).index(recipient)
                    ]
                    recipient_socket.send(
                        f"(Private) {nickname}: {message.decode('utf-8')[len(recipient) + 2:]}".encode(
                            "utf-8"
                        )
                    )
                else:
                ...
                broadcast(
                    f"{nickname}: {message.decode('utf-8')}", sender=client_socket
                )
    except:
        remove_client(client_socket)
