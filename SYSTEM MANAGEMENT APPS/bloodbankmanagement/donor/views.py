...
def donor_dashboard_view(request):
    ...
    dict = {
        "requestpending": bmodels.BloodRequest.objects.all()
        .filter(request_by_donor=donor)
        .filter(status="Pending")
        .count(),
        "requestapproved": bmodels.BloodRequest.objects.all()
        .filter(request_by_donor=donor)
        .filter(status="Approved")
        .count(),
        "requestmade": bmodels.BloodRequest.objects.all()
        .filter(request_by_donor=donor)
        .count(),
        "requestrejected": bmodels.BloodRequest.objects.all()
        .filter(request_by_donor=donor)
...
def request_history_view(request):
    ...
    return render(
        request, "donor/request_history.html", {"blood_request": blood_request}
    )
