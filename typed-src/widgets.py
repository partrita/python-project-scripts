...
class ClockFrame(ttk.Frame):
    def __init__(
        ...
        self.style.configure(
            style="Time.TLabel", font=("Helvetica", 30, "bold"), anchor="center"
        )

        # Configure the style of time label
        self.style.configure(
            style="Data.TLabel", font=("Helvetica", 30, "bold"), anchor="center"
        )

        # Creating the widgets for the time and the date

        self.label_time = ttk.Label(
            master=self, textvariable=self.time_string, style="Time.TLabel"
        )

        self.label_date = ttk.Label(
            master=self, textvariable=self.date_string, style="Data.TLabel"
        )
...
class AddAlarmClock(ttk.Frame):
    def __init__(self, parent, button_function):
        ...
        self.style = ttk.Style()
        self.style.configure(
            style="Alarm.TButton", font=("Helvetica", 20, "bold"), anchor="center"
        )
