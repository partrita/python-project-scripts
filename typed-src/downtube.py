...
def main():
    ...
    main_window.rowconfigure(
        0, weight=1
    )  # Rowconfiguration of root window in order to expand widgets, when window is is resized.
    ...
    linke_label = tk.Label(
        main_frame, text="Browse to save the file", bg="white", width=40, anchor="w"
    )
    ...
    resolution_box = ttk.Combobox(
        main_frame, textvariable=my_string_var, values=["360p", "720p"]
    )
