...
def process_target(queue: multiprocessing.Queue, input: str):
    ...
    if len(sub_logger.handlers) == 0 or not isinstance(
        sub_logger.handlers[0], logging.handlers.QueueHandler
    ):
    ...
    if is_pool:
        ...
        for i in ["a", "b", "c", "d", "e"]:
            worker = multiprocessing.get_context("fork").Process(
                target=process_target, args=(logger_helper.queue, i)
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-p",
        "--is_pool",
        help="is run in multiprocessing pool mode",
        default=True,
        type=lambda x: (str(x).lower() in ["true", "1", "yes", "y", "True"]),
    )
