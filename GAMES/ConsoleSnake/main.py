...
class Snake:
    ...
    def alive(self):
        ...
        head = self.parts[0]
        if (
            (head in self.parts[1:])
            or (not (0 < head[0] <= self.size[0]))
            or (not (0 < head[1] <= self.size[1]))
        ):
            return False
        return True

    def get_action(self, character):
        if (character in "wasd") and (
            self.get_opposites()[character] != self.dir or len(self.parts) == 1
        ):
            self.dir = character
        self.update_coors()
        self.check_fruit()
...
def main():
    ...
    update_time = (
        0.125  # This is how much time there is between updates, 1/update_time = fps
    )
    keep_playing = True
    kb = KBHit()
    while keep_playing:
