...
from helpers import (
    ...
    random_string_generator_only_alpha,
    is_valid_url,
)
...
def shorten_url():
    ...
    if first_keyword in reserved_keywords:
        flash(
            {
                "type": "error",
                "data": f"{first_keyword} is a reserved keyword! Please use any other word!",
            }
        )
        return redirect("/")
    incoming_data = {
    ...
    if url_details := url_data_collection.find_one(
        {"no_of_/": incoming_data["no_of_/"], "order": incoming_data["order"]}
    ):
...
def before_request_func():
    ...
    incoming_data = {"no_of_/": url.count("/"), "order": url.split("/")}
    if url_details := url_data_collection.find_one(
        {"no_of_/": incoming_data["no_of_/"], "order": incoming_data["order"]}
    ):
