        ...
        for i in range(self.dimension[0]):
            ...
            for j in range(self.dimension[1]):
                if ([i, j] in self.path) or ((i, j) in self.path):
                    doc.append(
                        "<td style='height:10px;width:10px;background-color:green;'></td>"
                    )
                elif ([i, j] in self.seen) or ((i, j) in self.seen):
                    doc.append(
                        "<td style='height:10px;width:10px;background-color:lightblue;'></td>"
                    )
                elif ([i, j] in self.walls) or ((i, j) in self.walls):
                    doc.append(
                        "<td style='height:10px;width:10px;background-color:black;'></td>"
                    )
                else:
                    doc.append(
                        "<td style='height:10px;width:10px;background-color:white;'></td>"
                    )
            doc.append("</tr>")
