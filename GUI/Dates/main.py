...
def workdays_between(date_one, date_two):
    ...
    try:
        ...
        result = np.busday_count(
            tmp_one.strftime("%Y-%m-%d"), tmp_two.strftime("%Y-%m-%d")
        )
    return result
...
def main_window():
    ...
    layout = [
        ...
        [sg.Text("Date one:", size=tsz), sg.Input("", size=isz, key="-DATE01-")],
        [sg.Text("Date two:", size=tsz), sg.Input("", size=isz, key="-DATE02-")],
        [sg.Text("Answer:", size=tsz), sg.Input("", size=isz, key="-OUTPUT-")],
...
while True:  # This is the main loop.
    ...
    if event == "-BETWEEN-":
        try:
            window["-OUTPUT-"].update(
                value=str(abs(days_between(values["-DATE01-"], values["-DATE02-"])))
            )
    ...
    if event == "-WORKING-":
        try:
            window["-OUTPUT-"].update(
                value=str(abs(workdays_between(values["-DATE01-"], values["-DATE02-"])))
            )
        except TypeError as err:
            sg.popup("Error:", err)
