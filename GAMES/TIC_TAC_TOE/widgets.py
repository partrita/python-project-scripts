...
class BoardGame(ttk.Frame):
        ...
        for rows in range(BOARD_SIZE[0]):
            for cols in range(BOARD_SIZE[1]):
                self.board_position[rows][cols] = Button(
                    ...
                    column=cols,
                    columnspan=1,
                    command=lambda row=rows, column=cols: self.player_move(
    ...
    def column_check(self):
        ...
        for column in range(BOARD_SIZE[1]):
            if (
                self.board_position[0][column]["text"]
    ...
    def row_check(self):
        ...
        for row in range(BOARD_SIZE[0]):
            if (
                self.board_position[row][0]["text"]
                == self.board_position[row][1]["text"]
    ...
    def check_first_diagonal(self) -> bool:
        ...
        """
        if (
            self.board_position[0][0]["text"]
            == self.board_position[1][1]["text"]
