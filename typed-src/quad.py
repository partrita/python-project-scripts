...
class Quadratic:
    ...
    def __init__(self, a: float, b: float, c: float) -> None:
        ...
        if (
            (type(a) == type(0.1) or type(a) == type(1))
            and (type(b) == type(0.1) or type(b) == type(1))
            and (type(c) == type(0.1) or type(c) == type(1))
        ):
            self.a = a
            self.b = b
            self.c = c
    ...
    def solveQuad(self) -> tuple[float, float]:
        ...
        ) ** 0.5  # The determinant of a quadratic equation is the root of b² - 4ac
