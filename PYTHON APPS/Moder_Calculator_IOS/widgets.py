...
class OutputLabel(ttk.Label):
    ...
    def __init__(
        self, parent, style: str, string_var, row: int, anchor: str, column: int = 0
    ) -> None:
        super().__init__(
            master=parent,
            style=style,
        ...
        self.grid(row=row, column=column, columnspan=4, sticky=anchor)
...
class Button(ttk.Button):
    ...
    def __init__(
        ...
        super().__init__(master=parent, text=text, style=style, command=func)
        self.grid(
            row=row,
            column=column,
            columnspan=span,
...
class NumberButtons(Button):
    def __init__(
        self, parent, text: str, style: str, func: str, row: int, column: int, span: int
    ) -> None:
        super().__init__(
            parent=parent,
            text=text,
