...
def admin_dashboard_view(request):
    ...
    dict = {
        ...
        "totalapprovedrequest": models.BloodRequest.objects.all()
...
def update_approve_status_view(request, pk):
    ...
    if stock.unit > unit:
        ...
        message = (
            "Stock Doest Not Have Enough Blood To Approve This Request, Only "
            + str(stock.unit)
            + " Unit Available"
        )
    req.save()
    ...
    requests = models.BloodRequest.objects.all().filter(status="Pending")
    return render(
        request, "blood/admin_request.html", {"requests": requests, "message": message}
    )
