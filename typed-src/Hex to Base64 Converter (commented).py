...
def hextodec(hexstring):
    ...
    for pair in hexlist:
        ...
        for char in hextodecinput:
            fhand.write(
                f"{hexdecalph.index(char)} is index of character in hexdecalph list\n"
            )
            fhand.write(f"{hextodecinput.index(char)} is index of character in input\n")
            fhand.write(
                f"{16 ** hextodecinput.index(char)} is 16 to the power of the index of the character in the input\n"
            )
            fhand.write(
                f"---Equation will be {hexdecalph.index(char)} * {16 ** hextodecinput.index(char)}\n"
            )
    ...
    for item in finaldeclist:
        ...
        fhand.write(
            f"{finalchar} is final character in base64 list using decimal conversion of 6-bit binary word as index\n"
        )
