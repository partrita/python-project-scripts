...
def resolve_perspective(
    source_image: np.ndarray, points: np.ndarray, target_shape: tuple[int, int]
) -> np.ndarray:
    ...
    transformation_matrix: cv2.typing.MatLike = cv2.getPerspectiveTransform(
        points.astype(np.float32), output_points
    )
    output: cv2.typing.MatLike = cv2.warpPerspective(
        source_image,
...
def get_grid_size(
    image: np.ndarray,
    boxes: list[list[int]],
    allowed_sizes: list[tuple[int, int]] = [(2, 3), (3, 3), (4, 4)],
) -> tuple[int, int]:
    h, w = image.shape
    for size in allowed_sizes:
        ...
        for box in boxes:
            _, x1, y1, x2, y2 = box
            if (
                abs(int(x1 / s1) - int(x2 / s1))
                + abs(int((h - y1) / s2) - int((h - y2) / s2))
            ) > 0:
                break
        else:
            return size
...
def get_points(
    image: np.ndarray, boxes: list[list[int]], grid_size: tuple[int, int]
...
def resolve_image(path: str) -> tuple[tuple, list[tuple[int, tuple]]]:
    ...
    *img_shape, _ = img.shape
    ...
    boxes = pt.image_to_boxes(
        output, "eng", config=r"-c tessedit_char_whitelist=0123456789 --psm 13 --oem 3"
    )
    print(boxes)
    h, w = output.shape
    new_boxes_str = ""
