...
def caption():
    ...
    if request.method == "POST":
        ...
        path = (
            "Student-Feedback-Sentiment-Analysis-main/static/Student-Feedback-Form.csv"
        )
        f.save(path)
        ...
        print(
            values[category_no - 1],
            labels[category_no - 1],
            category_no,
            category_no - 1,
        )

        if category_no == 1:
            results_1 = {"f1": "Teacher Feedback", "pos1": pos1, "n1": n1, "neg1": neg1}

            drawFigure.make(
                values[category_no - 1], labels[category_no - 1], category_no
            )

            return render_template("index1.html", result1=results_1, cat=category_no)

        elif category_no == 2:
            results_2 = {"f2": "Course Content", "pos2": pos2, "n2": n2, "neg2": neg2}
            ...
            results_4 = {"f4": "Laboratory", "pos4": pos4, "n4": n4, "neg4": neg4}
            ...
            for i in range(0, 6):
                ...
                for p in ax.patches:
                    ax.annotate(
                        "%.1f%%" % (100 * float(p.get_height() / sum(values[i]))),
                ...
                plt.savefig(
                    f"Student-Feedback-Sentiment-Analysis-main/static/plot{i+10}.jpg"
                )
