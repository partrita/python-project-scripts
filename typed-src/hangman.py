...
class Hangman:
    ...
    def set_difficulty(self):  # here you can configure different difficulties
        if self.difficulty == "easy":
            self.num_of_hint_words = round(
                len(self.word) * 0.4
            )  # amounts of hints equals to 40% of length
            self.attempts = 10
        elif self.difficulty == "normal":
            self.num_of_hint_words = round(
                len(self.word) * 0.3
            )  # amounts of hints equals to 30% of length
            self.attempts = 7
        elif self.difficulty == "hard":
            self.num_of_hint_words = round(
                len(self.word) * 0.2
            )  # amounts of hints equals to 20% of length
            self.attempts = 5
    ...
    def welcome_message(self):
        ...
        print(
            "Here you can try guessing letters of a randomly generated words based on hints!"
        )
    ...
    def input_guess(self):
        ...
        while self.game_active and self.attempts > 0:
            ...
            if self.user_guess in self.word:
                self.guess_display = [
                    self.user_guess if list(self.word)[i] == self.user_guess else char
                    for i, char in enumerate(self.guess_display)
                ]
                print("--------------------------------------")
                print(" ".join(self.guess_display))
...
try:
    my_hangman = Hangman(
        "fruit", "hard"
    )  # (Category, Difficulty) Categories: fruit, vegetable, animal. Difficulties: easy, normal, hard.
