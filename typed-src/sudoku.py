...
class Grid:
    ...
    def __init__(self, rows: int | None = None, columns: int | None = None) -> None:
        ...
        if self.grid_len >= 10:
            self.domain.extend(
                chr(ord("A") + i - 10) for i in range(10, self.grid_len + 1)
            )
        self.cells: list[Cell] = [
            Cell(i, self.domain) for i in range(self.grid_len * self.grid_len)
        ]
        self.unsolved_cells: list[int] = [
            i for i in range(self.grid_len * self.grid_len)
        ]
        self.solved_cells: list[int] = []
        self.initial_solved: list[int] = []
        self.initial_unsolved: list[int] = [
            i for i in range(self.grid_len * self.grid_len)
        ]
    ...
    def solve(self) -> None:
        ...
        while len(self.unsolved_cells) > 0:
            ...
            while i < len(self.solved_cells):
                ...
                r, c = int(self.solved_cells[i] / self.grid_len), int(
                    self.solved_cells[i] % self.grid_len
                )
        ...
        with open(filename, "r") as f:
            for i, line in enumerate(f):
                ...
                if i == 0:
                    ...
                    initial_solved_pairs = [
                        tuple(int(i) for i in a.split(","))
                        for a in line.replace("(", "").replace(")", "").split("|")
                    ]
                elif i == 2:
                    initial_unsolved_pairs = [
                        tuple(eval(i) for i in a.split(","))
                        for a in line.replace("(", "").replace(")", "").split("|")
                    ]
            f.close()
        self.grid_len = self.rows * self.columns
